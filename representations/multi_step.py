...
class MultiStep(torch.nn.Module):
    def __init__(
        ...
        self.encoder_update_freq = cfg.algos.multi_step.encoder_update_freq # how often to update the encoder
    ...
    '''
      MOD:
      curr changes: still update fw model every iteration, 
      but only update encoder every encoder_update_freq iterations
      (ms config file)
    '''
    def train_step(self, batch, epoch, train_step):
        ...
        # shoudl always update forward model:
        ...
        with torch.no_grad():
            ...
            ss_distances = torch.linalg.norm(ss_diffs, ord=1, dim=-1) #base case distances

        #predicted distance by fw model
        with torch.no_grad():

            #predicted latent state for all actions?
            ...
            #predicted latent state for comparison
            ...
            target_distances = torch.mean(
                torch.linalg.norm(pred_ox_encoded - pred_oy_encoded, ord=1, dim=-1), dim=-1
            )
