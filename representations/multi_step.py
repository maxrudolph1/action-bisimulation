...
class MultiStep(torch.nn.Module):
    ...
    def train_step(self, batch, epoch, train_step):
        ...
        obs_seq = torch.as_tensor(batch["kobs"], device="cuda")
        act_seq = torch.as_tensor(batch["kaction"], device="cuda")
        k_steps = obs_seq.shape[1]-1 #-1 for the obs we start on

        cumalative_fw_loss = 0
        ...
        #encode first obs
        ox_encoded_online = self.encoder(obs_seq[:,0]) 
        # oy_encoded_online = self.encoder(obs_y)
        with torch.no_grad():
            ox_encoded_target = self.target_encoder(obs_seq[:,0]) # target encoder z = \hat{phi}(x)
            ss_encoded_x = self.ss_encoder(obs_seq[:,0])
        ...
        for k in range(0, k_steps):
            act_k = act_seq[:,k]
            latent_forward_prediction = self.forward_model(curr_latent_state, act_k)
            
            #encoding the next target state?
            with torch.no_grad():
                target_latent_state = self.target_encoder(obs_seq[:,k+1])
        ...
        #post k steps:
        with torch.no_grad():
            oy_encoded_target = self.target_encoder(obs_seq[:,-1]) 
            ss_encoded_y = self.ss_encoder(obs_seq[:,-1])
            ss_diffs = (ss_encoded_x - ss_encoded_y)
            ss_distances = torch.linalg.norm(ss_diffs, ord=1, dim=-1)


        oy_encoded_online = self.encoder(obs_seq[:,-1])
        ox_oy_diffs = ox_encoded_online - oy_encoded_online
        distances = torch.linalg.norm(ox_oy_diffs, ord=1, dim=-1) #distance btwn online encoder output?

        #target distance btwn predicted & target latenedt states
        target_distances = torch.linalg.norm(curr_latent_state - oy_encoded_target, ord=1, dim=-1)


        #this is the cumalative multistep loss
        ...
        log = {
            ...
            "cur_ms_distance": distances.float().mean().detach().item(),
            "forward_ms_distance": target_distances.float().mean().detach().item(),
            "forward_model_loss": cumalative_fw_loss.detach().item(),
            "gamma": self.gamma,
        }
        return log
    ...
    def old_train_step(self, batch, epoch, train_step):
        if epoch < self.ss_train_warmup_epochs:
            return {}
