...
class Navigate2DPO(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            -1.0, 1.0, [2, window_size, window_size], np.float32
        )
    ...
    def reset(self):
        while True:
            ...
            min_center = self.size // 20 - 1
            max_center = self.size // 20 + 1
    ...
    def _get_obs(self, grid, pos=None, goal=None):
        ...
        window_top = pos[0] - self.window_size // 2
        window_bottom = window_top + self.window_size

        
        window_left = pos[1] - self.window_size // 2
        window_right = window_left + self.window_size

        obs_win = [window_top, window_bottom, window_left, window_right]
        
        obstacles = np.pad(grid[0, :, :], self.window_size//2, "constant", constant_values=1)
        obstacle_obs = (obstacles[obs_win[0]:obs_win[1], obs_win[2]:obs_win[3]])
        goal_vec = np.clip(goal -  (pos - self.window_size // 2), 0, self.window_size-1)
        goal_grid = np.zeros_like(obstacle_obs)
        goal_grid[goal_vec[0], goal_vec[1]] = 1
        obstacle_obs = (obstacles[obs_win[0]:obs_win[1], obs_win[2]:obs_win[3]])
        pos -= self.window_size // 2
        return np.stack([obstacle_obs, goal_grid], axis=0) * 2 - 1
