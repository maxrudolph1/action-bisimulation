...
class Navigate2D(gym.Env):
    ...
    def reset(self, options=None):
        while True:
            ...
            free_idx = free_idx[
                (np.linalg.norm(free_idx - start, ord=1, axis=-1) >= self.min_goal_dist) & (np.linalg.norm(free_idx - start, ord=1, axis=-1) <= self.max_goal_dist)
            ]
            ...
            if self.find_path() is not None:
                break
        if self.her_obs:
            obs = {'observation':self._get_obs(self.grid, self.pos, self.goal)[:2,:,:],
                   'achieved_goal':self._get_obs(self.grid, self.pos, self.pos)[[1],:,:],
                   'desired_goal':self._get_obs(self.grid, self.goal, self.goal)[[2],:,:]}
            return obs, {}
    ...
    def step(self, action):
        ...
        if self.sparse_reward:
            ...
            reward = -np.linalg.norm(self.pos - self.goal, ord=1)
        if (
            np.all(new_pos >= 0)
            ...
            if np.all(new_pos == self.goal):
                reward = 0 if self.sparse_reward else 10
        ...
        if self.her_obs:
            obs_grid = self._get_obs(self.grid, self.pos, self.goal)
            obs = {'observation':obs_grid[:2,:,:],
                   'achieved_goal':obs_grid[[1],:,:],
                   'desired_goal':obs_grid[[2],:,:]}
    ...
    def compute_reward_fn(self, achieved_goal, desired_goal, info):
        pass
