...
def make_env(env_kwargs=dict(num_obstacles=4, grid_size=20, static_goal=False, obstacle_diameter=2)):
    capture_video= False
    def thunk():
...
def main(cfg: DictConfig):
    ...
    if sb3.__version__ < "2.0":
        raise ValueError("Please upgrade to stable-baselines3>=2.0.0")
    assert cfg.rl.num_envs == 1, "vectorized envs are not supported at the moment"
    ...
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s" % ("\n".join([f"|{key}|{value}|" for key, value in OmegaConf.to_container(cfg.rl).items()])),
    )
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(env_kwargs=cfg.env) for i in range(cfg.rl.num_envs)]
    )
    assert isinstance(envs.single_action_space, gym.spaces.Discrete), "only discrete action space is supported"

    q_network = QNetwork(envs, cfg.encoder).to(device)
    optimizer = optim.Adam(q_network.parameters(), lr=cfg.rl.learning_rate)
    target_network = QNetwork(envs, cfg.encoder).to(device)
    ...
    for global_step in range(cfg.train_steps):
        # ALGO LOGIC: put action logic here
        epsilon = linear_schedule(cfg.rl.start_epsilon, cfg.rl.end_epsilon, cfg.rl.exploration_fraction * cfg.train_steps, global_step)
